...
export async function calculateSiteHealthScore(siteId: string): Promise<number> {
  ...
  const totalPossibleDays = await prisma.worker.count({ where: { site_id: siteId, is_active: true } }) * 7;
  const presentCount = attendanceRecords.filter(a => a.status === "PRESENT").length;
  const attendanceRate = totalPossibleDays > 0 ? (presentCount / totalPossibleDays) : 1.0;

  const totalTasks = await prisma.task.count({ where: { site_id: siteId } });
...
export async function calculateProjectRiskScore(projectId: string): Promise<"LOW" | "MEDIUM" | "HIGH" | "CRITICAL"> {
  let riskPoints = 0;
