...
export default function SupervisorDashboard() {
  ...
  return (
    <div className="cc-dashboard">
      {!hasProject ? (
        <motion.div 
          className="cc-card cc-onboard-card"
        ...
        <>
          ...
          <div className="cc-actions-section">
            <div className="cc-actions-grid-v3">
              {[
                { label: 'Attendance', subtitle: 'Mark presence', icon: Users, bg: 'rgba(10, 61, 145, 0.08)', color: '#0A3D91', click: () => navigate('/attendance') },
                { label: 'Submit DPR', subtitle: 'Daily report', icon: ClipboardCheck, bg: 'rgba(22, 163, 74, 0.08)', color: '#16A34A', click: () => navigate('/dpr') },
                { label: 'Request Material', subtitle: 'Order inventory', icon: Package, bg: 'rgba(245, 158, 11, 0.08)', color: '#F59E0B', click: () => navigate('/inventory?create=true') },
                { label: 'Create Task', subtitle: 'Assign new work', icon: Plus, bg: 'rgba(10, 61, 145, 0.08)', color: '#0A3D91', click: () => navigate('/tasks?create=true') }
              ].map((item, idx) => {
                ...
                return (
                  <motion.button 
                    ...
                    style={{ border: 'none', background: 'var(--cc-white)' }}
                  >
                    <div className="cc-act-icon-box" style={{ background: item.bg, color: item.color }}>
                      <Icon size={24} />
                    </div>
                    <div className="cc-act-text">
                      <span className="cc-act-title">{item.label}</span>
                      <span className="cc-act-sub">{item.subtitle}</span>
                    </div>
                  </motion.button>
                );
              })}
                ...
                <div className="cc-card cc-dense-list">
                  {todayTasks.length > 0 ? (
                    todayTasks.slice(0, 5).map((task) => (
                      <div key={task.id} className="cc-dense-row" onClick={() => navigate('/tasks')} style={{ cursor: 'pointer' }}>
                ...
                <div className="cc-section-block">
                  ...
                  <div className="cc-alerts-stack">
                    {alerts.map((alert, idx) => {
                      ...
                      return (
                        <div key={idx} className="cc-alert-row-v3" style={{ background: alert.bg, cursor: 'pointer' }} onClick={() => navigate(alert.link)}>
