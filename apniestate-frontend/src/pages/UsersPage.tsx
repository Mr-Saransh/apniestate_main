          ...
          filteredUsers.map((u, i) => (
            <div key={u.id || i} className={`flex items-center gap-3 px-4 py-3 ${i < filteredUsers.length - 1 ? "border-b border-border" : ""}`}>
              ...
              <div className="flex flex-col items-end gap-1 flex-shrink-0">
                ...
                <button className="text-[10px] text-muted-foreground hover:text-primary transition-colors">Edit</button>
              </div>
            </div>
          ))
        )}
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input type="tel" className="w-full mt-1 p-2 bg-muted border border-border rounded-lg text-sm outline-none focus:ring-1 focus:ring-primary" value={formPhone} onChange={e => setFormPhone(e.target.value)} placeholder="+92..." />
                  </div>
                </div>
              </div>
